...
# Personal hardening: print a visible warning before any hook installation.
# This surfaces the fact that hooks fire on every git commit, including in client repos.
_HOOK_INSTALL_WARNING = """
[graphify] HOOK INSTALL WARNING
  Git hooks will run graphify on EVERY commit and checkout in this repo.
  - Do NOT install hooks in client project repos — they will send client code to LLMs.
  - Only install in personal/internal repos where you control the corpus.
  - Set GRAPHIFY_ALLOWED_BACKENDS or --backend ollama to control which LLM receives files.
  To proceed anyway, re-run with GRAPHIFY_CONFIRM_HOOK_INSTALL=1
"""
...
def install(path: Path = Path(".")) -> str:
    ...
    # Personal hardening: require explicit confirmation before installing hooks.
    # Hooks fire on every git commit and can send files to external LLMs silently.
    import os as _os
    if not _os.environ.get("GRAPHIFY_CONFIRM_HOOK_INSTALL", "").strip():
        print(_HOOK_INSTALL_WARNING, file=__import__("sys").stderr)
        raise RuntimeError(
            "Hook install aborted. Set GRAPHIFY_CONFIRM_HOOK_INSTALL=1 to proceed."
        )
